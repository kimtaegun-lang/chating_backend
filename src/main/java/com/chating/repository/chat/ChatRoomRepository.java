...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
	...
	Optional<ChatRoom> findRoomByIds(@Param("user1") String user1, @Param("user2") String user2);
	 ...
	 // 본인이 포함된 채팅방 조회
	@Query("SELECT new com.chating.dto.chat.ChatRoomResDTO(c.roomId, CASE WHEN c.user1 = :userId THEN c.user2 ELSE c.user1 END, c.createdAt) FROM ChatRoom c WHERE c.user1 = :userId OR c.user2 = :userId ORDER BY c.createdAt DESC")
	List<ChatRoomResDTO> findMyChatRooms(@Param("userId") String userId);
